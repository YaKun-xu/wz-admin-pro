...
if ($_POST) {
    ...
    if ($action === 'add') {
        ...
        $sql = "INSERT INTO shop_videos (product_id, title, description, video_url, sort_order, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$product_id, $title, $description, $video_url, $sort_order, $status]);
        
        header('Location: video_tutorials.php?success=1');
        exit;
    }
    ...
    if ($action === 'edit') {
        ...
        $sql = "UPDATE shop_videos SET product_id = ?, title = ?, description = ?, video_url = ?, sort_order = ?, status = ?, updated_at = NOW() WHERE id = ?";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$product_id, $title, $description, $video_url, $sort_order, $status, $id]);
    ...
    if ($action === 'delete') {
        ...
        $sql = "DELETE FROM shop_videos WHERE id = ?";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$id]);
...
// 获取总数
$countSql = "SELECT COUNT(*) FROM shop_videos v $whereClause";
...
// 获取视频列表
$sql = "SELECT v.*, p.title as product_name 
        FROM shop_videos v 
...
<head>
    ...
    <title>视频教程管理 - 王者荣耀查战力后台管理系统</title>
...
<body>
    ...
    <div class="main-content-wrapper">
        <div class="main-content">
            ...
            <div class="page-header">
                <h1>
                    <i class="bi bi-camera-video me-3"></i>视频教程管理
                </h1>
                <p>管理视频教程内容</p>
            </div>
                    ...
                    <div class="stats-card">
                        <div class="d-flex align-items-center">
                            ...
                            <div class="flex-grow-1 ms-3">
                                <h3 class="mb-0"><?php echo number_format($totalVideos); ?></h3>
                                <p class="text-muted mb-0">总视频数</p>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-lg-3 col-md-6 mb-3">
                    <div class="stats-card">
                        <div class="d-flex align-items-center">
                            ...
                            <div class="flex-grow-1 ms-3">
                                <h3 class="mb-0"><?php echo number_format(count(array_filter($videos, function($v) { return $v['status']; }))); ?></h3>
                                <p class="text-muted mb-0">启用视频</p>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-lg-3 col-md-6 mb-3">
                    <div class="stats-card">
                        <div class="d-flex align-items-center">
                            ...
                            <div class="flex-grow-1 ms-3">
                                <h3 class="mb-0"><?php echo number_format(count(array_filter($videos, function($v) { return !$v['status']; }))); ?></h3>
                                <p class="text-muted mb-0">禁用视频</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="filter-card">
                <div class="row align-items-end">
                    <div class="col-md-8 mb-3">
                        <label for="search" class="form-label">搜索视频</label>
            ...
            <div class="table-card">
                ...
                <?php if (empty($videos)): ?>
                    <div class="text-center py-5">
                        ...
                        <h4 class="mt-3 text-muted">暂无视频数据</h4>
                        <p class="text-muted">点击"添加视频"按钮开始添加</p>
                    </div>
                <?php else: ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php if ($video['video_url']): ?>
                                            <a href="<?php echo htmlspecialchars($video['video_url']); ?>" target="_blank" class="video-url text-primary" title="<?php echo htmlspecialchars($video['video_url']); ?>">
                                                <i class="bi bi-link-45deg me-1"></i>查看链接
                                            </a>
                                        <?php else: ?>
                                    ...
                                    <td>
                                        <?php if ($video['product_name']): ?>
                                            <span class="badge bg-info"><?php echo htmlspecialchars($video['product_name']); ?></span>
                                        <?php else: ?>
                                            <span class="text-muted">通用</span>
                                        <?php endif; ?>
                                    </td>
                                    <td><?php echo $video['sort_order']; ?></td>
                                    <td>
                                        <span class="badge <?php echo $video['status'] ? 'badge-success' : 'badge-danger'; ?>">
                                            <?php echo $video['status'] ? '启用' : '禁用'; ?>
                                        </span>
                                    </td>
                                    <td><?php echo date('Y-m-d H:i', strtotime($video['created_at'])); ?></td>
                                    <td>
                                        <button type="button" class="btn btn-warning btn-sm me-2" onclick="editVideo(<?php echo htmlspecialchars(json_encode($video)); ?>)">
                                            <i class="bi bi-pencil"></i>
                                        </button>
                                        <button type="button" class="btn btn-danger btn-sm" onclick="deleteVideo(<?php echo $video['id']; ?>)">
                ...
                <form method="POST" id="addVideoForm">
                    <div class="modal-body" style="padding: 2rem;">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="add_title" name="title" required placeholder="请输入视频标题">
                        </div>
                        <div class="mb-3">
                            <label for="add_description" class="form-label fw-bold">
                                <i class="bi bi-file-text me-1"></i>视频描述
                            </label>
                            <textarea class="form-control" id="add_description" name="description" rows="4" placeholder="请输入视频描述（可选）"></textarea>
                        </div>
                        <div class="mb-3">
                            <label for="add_video_url" class="form-label fw-bold">
                                <i class="bi bi-camera-video me-1"></i>视频链接 <span class="text-danger">*</span>
                            </label>
                            <input type="url" class="form-control" id="add_video_url" name="video_url" required placeholder="https://example.com/video.mp4">
                            <small class="form-text text-muted">
                                <i class="bi bi-link-45deg me-1"></i>请输入视频的完整URL地址
                            </small>
                        </div>
                        <div class="row">
                    ...
                    <div class="modal-footer" style="border-top: 1px solid #e9ecef; padding: 1.5rem 2rem; border-radius: 0 0 20px 20px;">
                        ...
                        <button type="submit" class="btn btn-primary">
                            <i class="bi bi-check-lg me-2"></i>添加视频
                        </button>
                    </div>
                </form>
            </div>
    ...
    <div class="modal fade" id="editVideoModal" tabindex="-1">
        <div class="modal-dialog modal-lg">
            <div class="modal-content" style="border-radius: 20px; border: none; box-shadow: 0 20px 60px rgba(0, 0, 0, 0.3);">
                <div class="modal-header" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 20px 20px 0 0; border: none; padding: 1.5rem 2rem;">
                    <h5 class="modal-title text-white" style="font-size: 1.5rem; font-weight: 700;">
                        <i class="bi bi-pencil-square me-2"></i>编辑视频
                    </h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form method="POST">
                    <div class="modal-body" style="padding: 2rem;">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="edit_title" name="title" required placeholder="请输入视频标题">
                        </div>
                        <div class="mb-3">
                            <label for="edit_description" class="form-label fw-bold">
                                <i class="bi bi-file-text me-1"></i>视频描述
                            </label>
                            <textarea class="form-control" id="edit_description" name="description" rows="4" placeholder="请输入视频描述（可选）"></textarea>
                        </div>
                        <div class="mb-3">
                            <label for="edit_video_url" class="form-label fw-bold">
                                <i class="bi bi-camera-video me-1"></i>视频链接 <span class="text-danger">*</span>
                            </label>
                            <input type="url" class="form-control" id="edit_video_url" name="video_url" required placeholder="https://example.com/video.mp4">
                            <small class="form-text text-muted">
                                <i class="bi bi-link-45deg me-1"></i>请输入视频的完整URL地址
                            </small>
                        </div>
    ...
    <script>
        ...
        function editVideo(video) {
            ...
            document.getElementById('edit_product_id').value = video.product_id || '';
            ...
            document.getElementById('edit_status').value = video.status;
        ...
        if (addVideoModal) {
            addVideoModal.addEventListener('hidden.bs.modal', function() {
